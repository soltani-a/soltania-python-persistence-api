#soltania-python-persistence-api🗄️ **Python Persistence API**: A JPA (Java Persistence API) and Spring Data inspired abstraction layer.
...
##📑 Table of Contents* [Architecture & Concepts](https://www.google.com/search?q=%23-architecture--concepts)
* [Installation](https://www.google.com/search?q=%23-installation)
* [Configuration Guide](https://www.google.com/search?q=%23-configuration-guide)
* [Usage](https://www.google.com/search?q=%23-usage)
* [Running the Demos (Paris Metro)](https://www.google.com/search?q=%23-running-the-demos)
* [Tests](https://www.google.com/search?q=%23-tests)
* [Project Structure](https://www.google.com/search?q=%23-project-structure)

---

##🏗 Architecture & ConceptsThis project ports solid concepts from the Java ecosystem to Python:
...
```bash
git clone https://github.com/your-user/soltania-python-persistence-api.git
...
##⚙️ Configuration GuideThis project uses a hierarchical configuration system inspired by Spring Boot. You do not need to modify the code to switch environments (Dev, Test, Prod).
...
###🚀 Source Priority (Hierarchy)The active value is determined by this priority order (from highest to lowest):
...
###🛠️ Configuration Methods (Examples)**1. Via CLI Arguments (Highest Priority)**
...
##💻 Usage###1. Define an EntityInherit from `BaseEntity`. Pydantic handles validation.
...
###2. Use a RepositoryNever manipulate the database directly. Use the repository.
...
##🚇 Running the DemosThe project includes a demo application modeling the **Paris Metro** (Lines 1 and 9).

###1. Populate the Database (Migration)This script creates the stations (Vertices) and connections (Edges) in your Gremlin server.
...
###2. Calculate an ItineraryThis script uses the shortest path algorithm via the Repository.
...
##🧪 TestsTests are managed by `pytest` and divided into two categories.

###Unit TestsTest the logic of models and repositories using **Mocks** (no DB connection required).
...
###Integration TestsActually connect to the configured Gremlin server to validate read/write operations.
